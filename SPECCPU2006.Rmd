...
Note the log scale which range over a factor of 100. Roughly speaking computing performance has increased between a factor of ten and one hundred since 2006, when the benchmark was first introduced.  Based on a visual analysis core count seems to play a role in the performance increase, but other factors may as well. For instance processor frequency has long been associated with performance gains and has an obvious and direct correlation to processing speed. 

However, this is a broad brush look at the data. Within it there are different types of chips and system configurations. 
